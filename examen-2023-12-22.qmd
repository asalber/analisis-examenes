...
:::{#exr-4}
Calcular el volumen de un helado formado por un cono de barquillo con ecuación $2x^2+2y^2-z^2=0$ sobre el que se coloca semiesfera de ecuación $x^2+y^2+(z-2)^2 = 1$ como se muestra en la figura. ¿Qué cantidad de barquillo se necesita para construir el cono del helado?
...
:::{.callout-tip collapse="true"}
## Solución
Despejando $z$ de ambas ecuaciones, se tiene que la función que define el cono es $f(x,y) = 2\sqrt{x^2+y^2}$ y la que define la semiesfera superior es $g(x,y) = 2 + \sqrt{1-x^2-y^2}$, de manera que el volumen del helado será el volumen comprendido entre las superficies de $g$ y $f$.
